...
const PatientDemographics = () => {
  ...
  // ✅ Call this function whenever `data` changes
  useEffect(() => {
    if (data.length > 0) {
      categorizeAges(data);
    }
  ...
  const categorizeAges = (patients) => {
    ...
    patients.forEach((patient) => {
      const age = Number(patient.Age); // make sure it's a number
      if (!isNaN(age)) {
  ...
  return (
    <div style={{ background: colors.dark, minHeight: "100vh", padding: 20 }}>
      <div
        style={{
          display: "grid",
          gap: 20,

...
export default function LipidsLine({ data }) {
  ...
  return (
    <div className="w-full p-4 sm:p-5 shadow-lg bg-white border border-green-300 rounded-xl">
      ...
      <div className="flex flex-col sm:flex-row flex-wrap items-center gap-3 sm:gap-6 mb-4 sm:mb-6">
        ...
        <div className="flex flex-col flex-1 ">
          <label className="text-green-900 font-medium text-sm mb-1">Age Range</label>
          <select
            value={ageRange}
            onChange={(e) => setAgeRange(e.target.value)}
            className="px-2 sm:px-3 py-2 border border-green-400 rounded-lg shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 text-sm sm:text-base"
          >
            {AGE_OPTIONS.map((opt) => (
        ...
        <div className="flex flex-col flex-1 ">
          <label className="text-green-900 font-medium text-sm mb-1">Gender</label>
          <select
            value={gender}
      ...
      <div className="w-full h-[280px] sm:h-[350px] md:h-[400px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            <XAxis dataKey="index" stroke="#064e3b" tick={{ fill: "#064e3b", fontSize: 12 }} />
            <YAxis stroke="#064e3b" tick={{ fill: "#064e3b", fontSize: 12 }} />
            <Tooltip
              contentStyle={{
                borderRadius: "10px",
                padding: "10px",
